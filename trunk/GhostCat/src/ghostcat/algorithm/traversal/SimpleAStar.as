package ghostcat.algorithm.traversal
{
	...
	public class SimpleAStar
	{
		...
		//====================================
		//地图模型
		private var m_mapTileModel : IMapModel;
		//最大寻路步数，限制超时返回
		 ...
		 */		
		public function SimpleAStar(p_mapTileModel :IMapModel, p_maxTry : int = 5000)
		{
		...
		{
			var p_startX : int = start.x;
			var p_startY : int = start.y;
			var p_endX : int = end.x;
			var p_endY : int = end.y; 
			...
			while (this.m_openCount > 0)
			{
				...
				aroundNotes = m_mapTileModel.getArounds(new Point(currNoteX, currNoteY));
				//对于周围的每一个节点
