	...
	public class Cataclasm extends GTransfer
	{
		...
		public function createTris():void
		{
			...
			for (j = 0; j < points.length - 1; j++)
			{
				...
				{
					var t1:Tri = new Tri(points[j][i],points[j][i + 1],points[j + 1][i],bitmapData,rect.topLeft)
					addChild(t1);
					var t2:Tri = new Tri(points[j][i + 1],points[j + 1][i],points[j + 1][i + 1],bitmapData,rect.topLeft);
					addChild(t2);
					tris.push(t1,t2);
				}
			}
...
class Tri extends Sprite
{
	...
	public function Tri(p1:Point,p2:Point,p3:Point,texture:BitmapData,offest:Point):void
	{
		...
		this.x = center.x + offest.x;
		this.y = center.y + offest.y;
		this.offest = offest;
		render();
	}
